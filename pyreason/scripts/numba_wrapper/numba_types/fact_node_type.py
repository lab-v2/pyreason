...
def type_fact(context):
    def typer(component, l, bnd, t_lower, t_upper, static):
        if isinstance(component, types.UnicodeType) and isinstance(l, label.LabelType) and isinstance(bnd, interval.IntervalType) and isinstance(t_lower, numba.types.Integer) and isinstance(t_upper, numba.types.Integer) and isinstance(static, numba.types.Boolean):
            return fact_type
    return typer
...
@lower_builtin(Fact, numba.types.string, label.label_type, interval.interval_type, numba.types.int8, numba.types.int8, numba.types.boolean)
