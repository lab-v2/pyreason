...
def type_fact(context):
    def typer(name, component, label_param, bnd, t_lower, t_upper, static):
        if isinstance(name, types.UnicodeType) and isinstance(component, types.UnicodeType) and isinstance(label_param, label.LabelType) and isinstance(bnd, interval.IntervalType) and isinstance(t_lower, numba.types.Integer) and isinstance(t_upper, numba.types.Integer) and isinstance(static, numba.types.Boolean):
            return fact_type
    return typer
