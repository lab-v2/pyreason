        ...
        if graph_attribute_parsing:
            for n in self.graph.nodes:
                for key, value in self.graph.nodes[n].items():
                    ...
                    f = fact_node.Fact('graph-attribute-fact', n, label.Label(l), interval.closed(l_bnd, 1), 0, 0,
                                       static=True)
                    facts_node.append(f)
            for e in self.graph.edges:
                for key, value in self.graph.edges[e].items():
                    ...
                    f = fact_edge.Fact('graph-attribute-fact', (e[0], e[1]), label.Label(l), interval.closed(l_bnd, 1),
                                       0, 0, static=True)
                    facts_edge.append(f)
