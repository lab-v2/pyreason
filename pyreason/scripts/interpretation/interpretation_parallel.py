# Changed: parallel entrypoint — same pure-Python engine as interpretation.py.
# (Used to be the `parallel=True` njit twin. Numba's gone, so it's just a copy.)
		...
		while timestep_loop:
				...
				if facts_to_be_applied_node[i][0] == t:
					...
					if l in interpretations_node[comp].world and interpretations_node[comp].world[l].is_static():
						...
						if (save_graph_attributes_to_rule_trace or not graph_attribute) and store_interpretation_changes:
							...
							for p1, p2 in ipl:
								if p1==l:
									rule_trace_node.append((int(t), int(fp_cnt), comp, p2, interpretations_node[comp].world[p2], True, 'IPL', f'IPL: {l.get_value()}', ''))
									if atom_trace:
										_update_rule_trace(rule_trace_node_atoms, [], [], interpretations_node[comp].world[p2], facts_to_be_applied_node_trace[i])
								elif p2==l:
									rule_trace_node.append((int(t), int(fp_cnt), comp, p1, interpretations_node[comp].world[p1], True, 'IPL', f'IPL: {l.get_value()}', ''))
									if atom_trace:
										_update_rule_trace(rule_trace_node_atoms, [], [], interpretations_node[comp].world[p1], facts_to_be_applied_node_trace[i])
				...
				if facts_to_be_applied_edge[i][0]==t:
					...
					if l in interpretations_edge[comp].world and interpretations_edge[comp].world[l].is_static():
						...
						if (save_graph_attributes_to_rule_trace or not graph_attribute) and store_interpretation_changes:
							...
							rule_trace_edge.append((int(t), int(fp_cnt), comp, l, interpretations_edge[comp].world[l], True, 'Fact', meta_name, ''))
							if atom_trace:
								_update_rule_trace(rule_trace_edge_atoms, [], [], bnd, facts_to_be_applied_edge_trace[i])
							for p1, p2 in ipl:
								if p1==l:
									rule_trace_edge.append((int(t), int(fp_cnt), comp, p2, interpretations_edge[comp].world[p2], True, 'IPL', f'IPL: {l.get_value()}', ''))
									if atom_trace:
										_update_rule_trace(rule_trace_edge_atoms, [], [], interpretations_edge[comp].world[p2], facts_to_be_applied_edge_trace[i])
								elif p2==l:
									rule_trace_edge.append((int(t), int(fp_cnt), comp, p1, interpretations_edge[comp].world[p1], True, 'IPL', f'IPL: {l.get_value()}', ''))
									if atom_trace:
										_update_rule_trace(rule_trace_edge_atoms, [], [], interpretations_edge[comp].world[p1], facts_to_be_applied_edge_trace[i])
					else:
			...
			while in_loop:
				...
				if update:
					...
					# Threadsafe flags for in_loop and update within range; merge after loop
					in_loop_threadsafe = []
					update_threadsafe = []
