...
class Interpretation:
	...
	def __init__(self, graph, ipl, annotation_functions, reverse_graph, atom_trace, save_graph_attributes_to_rule_trace, persistent, inconsistency_check, store_interpretation_changes, update_mode, allow_ground_rules, ad_hoc_grounding, resolution_levels, step_size):
		self.graph = graph
		self.ipl = ipl
			...
			for i in range(len(facts_to_be_applied_edge)):
				if facts_to_be_applied_edge[i][0]==t:
					...
					if comp not in edges_set:
						_add_edge(comp[0], comp[1], neighbors, reverse_neighbors, nodes, edges, label.Label(''), interpretations_node, interpretations_edge, predicate_map_edge, numba.typed.List.empty_list(label.label_type), num_ga, t)
						edges_set.add(comp)
								...
								if ad_hoc_grounding:
									...
									if 'field' in comp[0]:
										...
										directions = numba.typed.List(
											[label.Label('fastUp'), label.Label('fastDown'),
											 label.Label('fastLeft'),
	...
	def add_edge(self, edge, l):
		...
		_add_edge(edge[0], edge[1], self.neighbors, self.reverse_neighbors, self.nodes, self.edges, l, self.interpretations_node, self.interpretations_edge, self.predicate_map_edge, numba.typed.List.empty_list(label.label_type), self.num_ga, -1)
		...
		if rule_type == 'node':
			...
			for valid_e in valid_edge_groundings:
				...
				if add_head_edge_to_graph and (head_var_1, head_var_2) == (head_var_1_grounding, head_var_2_grounding):
					_add_edge(head_var_1, head_var_2, neighbors, reverse_neighbors, nodes, edges, label.Label(''), interpretations_node, interpretations_edge, predicate_map_edge, numba.typed.List.empty_list(label.label_type), num_ga, t)
...
@numba.njit(cache=True)
def _add_edge(source, target, neighbors, reverse_neighbors, nodes, edges, l, interpretations_node, interpretations_edge, predicate_map, node_labels, num_ga, t):
...
def _add_edges(sources, targets, neighbors, reverse_neighbors, nodes, edges, l, interpretations_node, interpretations_edge, predicate_map, num_ga, t):
	...
	for source in sources:
		for target in targets:
			edge, new_edge = _add_edge(source, target, neighbors, reverse_neighbors, nodes, edges, l, interpretations_node, interpretations_edge, predicate_map, numba.typed.List.empty_list(label.label_type), num_ga, t)
