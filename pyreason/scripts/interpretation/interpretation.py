...
class Interpretation:
    ...
    def __init__(self,
        ...
        # One to One correspondence between rules_to_be_applied_node
        # and rules_to_be_applied_node_trace if atom_trace is true
        self.rules_to_be_applied_node_trace = numba.typed.List.empty_list(numba.types.Tuple((numba.types.ListType(
            list_of_nodes), numba.types.ListType(numba.types.ListType(edge_type)), numba.types.string)))
        self.rules_to_be_applied_edge_trace = numba.typed.List.empty_list(
            numba.types.Tuple((numba.types.ListType(list_of_nodes), numba.types.string)))
        ...
        self.rules_to_be_applied_node = numba.typed.List.empty_list(
            numba.types.Tuple((numba.types.int8, node_type, label.label_type, interval.interval_type)))
        self.rules_to_be_applied_edge = numba.typed.List.empty_list(
            numba.types.Tuple((numba.types.int8, edge_type, label.label_type, interval.interval_type)))
        self.facts_to_be_applied_node = numba.typed.List.empty_list(numba.types.Tuple((numba.types.int8, node_type,
                                                                                       label.label_type,
                                                                                       interval.interval_type,
                                                                                       numba.types.boolean,
                                                                                       numba.types.boolean)))
        self.facts_to_be_applied_edge = numba.typed.List.empty_list(numba.types.Tuple((numba.types.int8, edge_type,
                                                                                       label.label_type,
                                                                                       interval.interval_type,
                                                                                       numba.types.boolean,
                                                                                       numba.types.boolean)))
        self.edges_to_be_added_node_rule = numba.typed.List.empty_list(
            numba.types.Tuple((list_of_nodes, list_of_nodes, label.label_type)))
        self.edges_to_be_added_edge_rule = numba.typed.List.empty_list(
            numba.types.Tuple((list_of_nodes, list_of_nodes, label.label_type)))

        # Keep track of all the rules that have affected each node/edge at each timestep/fp operation,
        # and all ground atoms that have affected the rules as well.
        # Keep track of previous bounds and name of the rule/fact here
        self.rule_trace_node_atoms = numba.typed.List.empty_list(numba.types.Tuple((numba.types.ListType(
            list_of_nodes), numba.types.ListType(numba.types.ListType(edge_type)),
                                                                                    interval.interval_type,
                                                                                    numba.types.string)))
        self.rule_trace_edge_atoms = numba.typed.List.empty_list(numba.types.Tuple(
            (numba.types.ListType(list_of_nodes), interval.interval_type, numba.types.string)))
        self.rule_trace_node = numba.typed.List.empty_list(numba.types.Tuple(
            (numba.types.int8, numba.types.int8, node_type, label.label_type, interval.interval_type)))
        self.rule_trace_edge = numba.typed.List.empty_list(numba.types.Tuple(
            (numba.types.int8, numba.types.int8, edge_type, label.label_type, interval.interval_type)))
        ...
        self.interpretations_node = self._init_interpretations_node(nodes=numba.typed.List(self.graph.nodes()),
                                                                    available_labels=self.available_labels_node,
                                                                    specific_labels=self.specific_node_labels)
        self.interpretations_edge = self._init_interpretations_edge(edges=numba.typed.List(self.graph.edges()),
                                                                    available_labels=self.available_labels_edge,
                                                                    specific_labels=self.specific_edge_labels)

        # Setup graph neighbors
        self.neighbors = numba.typed.Dict.empty(key_type=node_type,
                                                value_type=list_of_nodes)
    ...
    def _init_interpretations_node(nodes: numba.typed.List,
                                   available_labels:  numba.typed.List,
                                   specific_labels: numba.typed.Dict) -> numba.typed.Dict:
    ...
    @staticmethod
    @numba.njit(cache=True)
    def _init_interpretations_edge(edges: numba.typed.List,
                                   available_labels:  numba.typed.List,
                                   specific_labels: numba.typed.Dict) -> numba.typed.Dict:
        interpretations = numba.typed.Dict.empty(key_type=edge_type,
                                                 value_type=world.world_type)
    ...
    def start_fp(self,
                 ...
                 verbose: bool):
        max_facts_time = self._init_facts(facts_node=facts_node,
                                          facts_edge=facts_edge,
                                          facts_to_be_applied_node=self.facts_to_be_applied_node,
                                          facts_to_be_applied_edge=self.facts_to_be_applied_edge,
                                          facts_to_be_applied_node_trace=self.facts_to_be_applied_node_trace,
                                          facts_to_be_applied_edge_trace=self.facts_to_be_applied_edge_trace,
                                          atom_trace=self.atom_trace)
        self._start_fp(rules=rules,
                       max_facts_time=max_facts_time,
                       verbose=verbose)

    @staticmethod
    @numba.njit(cache=True)
    def _init_facts(facts_node: numba.typed.List,
                    facts_edge: numba.typed.List,
                    facts_to_be_applied_node: numba.typed.List,
                    facts_to_be_applied_edge: numba.typed.List,
                    facts_to_be_applied_node_trace: numba.typed.List,
                    facts_to_be_applied_edge_trace: numba.typed.List,
                    atom_trace: bool) -> int:
        max_time = 0
        ...
        for fact in facts_node:
            for t in range(fact.get_time_lower(), fact.get_time_upper() + 1):
                ...
                facts_to_be_applied_node.append((numba.types.int8(t), fact.get_component(), fact.get_label(),
                                                 fact.get_bound(), fact.static, graph_attribute))
                if atom_trace:
        ...
        for fact in facts_edge:
            for t in range(fact.get_time_lower(), fact.get_time_upper() + 1):
                ...
                name = fact.get_name()
                graph_attribute = True if name == 'graph-attribute-fact' else False
                facts_to_be_applied_edge.append((numba.types.int8(t), fact.get_component(), fact.get_label(),
                                                 fact.get_bound(), fact.static, graph_attribute))
                if atom_trace:
    ...
    def _start_fp(self,
                  ...
                  max_facts_time: int,
                  verbose: bool):
        fp_cnt, t = self.reason(interpretations_node=self.interpretations_node,
                                interpretations_edge=self.interpretations_edge,
                                tmax=self.tmax,
                                rules=rules,
                                nodes=numba.typed.List(self.graph.nodes()),
                                edges=numba.typed.List(self.graph.edges()),
                                neighbors=self.neighbors,
                                rules_to_be_applied_node=self.rules_to_be_applied_node,
                                rules_to_be_applied_edge=self.rules_to_be_applied_edge,
                                edges_to_be_added_node_rule=self.edges_to_be_added_node_rule,
                                edges_to_be_added_edge_rule=self.edges_to_be_added_edge_rule,
                                rules_to_be_applied_node_trace=self.rules_to_be_applied_node_trace,
                                rules_to_be_applied_edge_trace=self.rules_to_be_applied_edge_trace,
                                facts_to_be_applied_node=self.facts_to_be_applied_node,
                                facts_to_be_applied_edge=self.facts_to_be_applied_edge,
                                facts_to_be_applied_node_trace=self.facts_to_be_applied_node_trace,
                                facts_to_be_applied_edge_trace=self.facts_to_be_applied_edge_trace,
                                labels_node=self.available_labels_node,
                                labels_edge=self.available_labels_edge,
                                specific_labels_node=self.specific_node_labels,
                                specific_labels_edge=self.specific_edge_labels,
                                ipl=self.ipl,
                                rule_trace_node=self.rule_trace_node,
                                rule_trace_edge=self.rule_trace_edge,
                                rule_trace_node_atoms=self.rule_trace_node_atoms,
                                rule_trace_edge_atoms=self.rule_trace_edge_atoms,
                                reverse_graph=self.reverse_graph,
                                atom_trace=self.atom_trace,
                                save_graph_attributes_to_rule_trace=self.save_graph_attributes_to_rule_trace,
                                max_facts_time=max_facts_time,
                                convergence_mode=self._convergence_mode,
                                convergence_delta=self._convergence_delta,
                                verbose=verbose)
        self.time = t
    ...
    @staticmethod
    @numba.njit(cache=True)
    def reason(interpretations_node: numba.typed.Dict,
               interpretations_edge: numba.typed.Dict,
               tmax: int,
               rules: numba.typed.List,
               nodes: numba.typed.List,
               edges: numba.typed.List,
               neighbors: numba.typed.Dict,
               rules_to_be_applied_node: numba.typed.List,
               rules_to_be_applied_edge: numba.typed.List,
               edges_to_be_added_node_rule: numba.typed.List,
               edges_to_be_added_edge_rule: numba.typed.List,
               rules_to_be_applied_node_trace: numba.typed.List,
               rules_to_be_applied_edge_trace: numba.typed.List,
               facts_to_be_applied_node: numba.typed.List,
               facts_to_be_applied_edge: numba.typed.List,
               facts_to_be_applied_node_trace: numba.typed.List,
               facts_to_be_applied_edge_trace: numba.typed.List,
               labels_node: numba.typed.List,
               labels_edge: numba.typed.List,
               specific_labels_node: numba.typed.Dict,
               specific_labels_edge: numba.typed.Dict,
               ipl: numba.typed.List,
               rule_trace_node: numba.typed.List,
               rule_trace_edge: numba.typed.List,
               rule_trace_node_atoms: numba.typed.List,
               rule_trace_edge_atoms: numba.typed.List,
               reverse_graph: bool,
               atom_trace: bool,
               save_graph_attributes_to_rule_trace: bool,
               max_facts_time: int,
        ...
        for t in range(tmax + 1):
            ...
            for i in range(len(facts_to_be_applied_node)):
                if facts_to_be_applied_node[i][0] == t:
                    comp, l, bnd, static, graph_attribute = facts_to_be_applied_node[i][1], \
                        facts_to_be_applied_node[i][2], facts_to_be_applied_node[i][3], \
                        facts_to_be_applied_node[i][4], facts_to_be_applied_node[i][5]
                    # Check if bnd is static. Then no need to update, just add to rule trace, 
                    # check if graph attribute and add ipl complement to rule trace as well
                    if interpretations_node[comp].world[l].is_static():
                        ...
                        if save_graph_attributes_to_rule_trace or not graph_attribute:
                            ...
                            if atom_trace:
                                _update_rule_trace_node(rule_trace_node_atoms, numba.typed.List.empty_list(
                                    numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(
                                    numba.typed.List.empty_list(edge_type)), bnd, facts_to_be_applied_node_trace[i])
                            for p1, p2 in ipl:
                                if p1 == l:
                                    rule_trace_node.append((numba.types.int8(t), numba.types.int8(fp_cnt), comp, p2,
                                                            interpretations_node[comp].world[p2]))
                                    if atom_trace:
                                        _update_rule_trace_node(rule_trace_node_atoms, numba.typed.List.empty_list(
                                            numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(
                                            numba.typed.List.empty_list(edge_type)),
                                                                interpretations_node[comp].world[p2],
                                                                facts_to_be_applied_node_trace[i])
                                elif p2 == l:
                                    rule_trace_node.append((numba.types.int8(t), numba.types.int8(fp_cnt), comp, p1,
                                                            interpretations_node[comp].world[p1]))
                                    if atom_trace:
                                        _update_rule_trace_node(rule_trace_node_atoms, numba.typed.List.empty_list(
                                            numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(
                                            numba.typed.List.empty_list(edge_type)),
                        ...
                        if check_consistent_node(interpretations_node, comp, (l, bnd)):
                            ...
                            u, changes = _update_node(interpretations_node, comp, (l, bnd), ipl, rule_trace_node,
                                                      fp_cnt, t, static, convergence_mode, atom_trace,
                                                      save_graph_attributes_to_rule_trace,
                                                      rules_to_be_applied_node_trace, i, facts_to_be_applied_node_trace,
                                                      rule_trace_node_atoms, mode=mode)
                        ...
                        # Resolve inconsistency
                            ...
                            resolve_inconsistency_node(interpretations_node, comp, (l, bnd), ipl, t, fp_cnt, atom_trace,
                                                       rule_trace_node, rule_trace_node_atoms)
                    if static:
                        facts_to_be_applied_node[i] = (
                            numba.types.int8(facts_to_be_applied_node[i][0] + 1), comp, l, bnd, static, graph_attribute)
            ...
            for i in range(len(facts_to_be_applied_edge)):
                if facts_to_be_applied_edge[i][0] == t:
                    comp, l, bnd, static, graph_attribute = facts_to_be_applied_edge[i][1], facts_to_be_applied_edge[i][
                        2], facts_to_be_applied_edge[i][3], facts_to_be_applied_edge[i][4], facts_to_be_applied_edge[i][
                        5]
                    # Check if bnd is static. Then no need to update, just add to rule trace, 
                    # check if graph attribute, and add ipl complement to rule trace as well
                    if interpretations_edge[comp].world[l].is_static():
                        # Inverse of this is: if not save_graph_attributes_to_rule_trace and graph_attribute
                        if save_graph_attributes_to_rule_trace or not graph_attribute:
                            rule_trace_edge.append((numba.types.int8(t), numba.types.int8(fp_cnt), comp, l,
                                                    interpretations_edge[comp].world[l]))
                            if atom_trace:
                                _update_rule_trace_edge(rule_trace_edge_atoms, numba.typed.List.empty_list(
                                    numba.typed.List.empty_list(node_type)), bnd, facts_to_be_applied_edge_trace[i])
                            for p1, p2 in ipl:
                                if p1 == l:
                                    rule_trace_edge.append((numba.types.int8(t), numba.types.int8(fp_cnt), comp, p2,
                                                            interpretations_edge[comp].world[p2]))
                                    if atom_trace:
                                        _update_rule_trace_edge(rule_trace_edge_atoms, numba.typed.List.empty_list(
                                            numba.typed.List.empty_list(node_type)),
                                                                interpretations_edge[comp].world[p2],
                                                                facts_to_be_applied_edge_trace[i])
                                elif p2 == l:
                                    rule_trace_edge.append((numba.types.int8(t), numba.types.int8(fp_cnt), comp, p1,
                                                            interpretations_edge[comp].world[p1]))
                                    if atom_trace:
                                        _update_rule_trace_edge(rule_trace_edge_atoms, numba.typed.List.empty_list(
                                            numba.typed.List.empty_list(node_type)),
                                                                interpretations_edge[comp].world[p1],
                        ...
                        if check_consistent_edge(interpretations_edge, comp, (l, bnd)):
                            mode = 'graph-attribute-fact' if graph_attribute else 'fact'
                            u, changes = _update_edge(interpretations_edge, comp, (l, bnd), ipl, rule_trace_edge,
                                                      fp_cnt, t, static, convergence_mode, atom_trace,
                                                      save_graph_attributes_to_rule_trace,
                                                      rules_to_be_applied_edge_trace, i, facts_to_be_applied_edge_trace,
                                                      rule_trace_edge_atoms, mode=mode)
                        ...
                        # Resolve inconsistency
                            ...
                            resolve_inconsistency_edge(interpretations_edge, comp, (l, bnd), ipl, t, fp_cnt, atom_trace,
                                                       rule_trace_edge, rule_trace_edge_atoms)
                    if static:
                        facts_to_be_applied_edge[i] = (
                            numba.types.int8(facts_to_be_applied_edge[i][0] + 1), comp, l, bnd, static, graph_attribute)
            ...
            while in_loop:
                ...
                for idx, i in enumerate(rules_to_be_applied_node):
                    if i[0] == t:
                        ...
                        edges_added, changes = _add_edges(sources, targets, neighbors, nodes, edges, edge_l,
                                                          interpretations_node, interpretations_edge)
                        changes_cnt += changes

                        # Update bound for newly added edges. Use bnd to update all edges if label is specified, 
                        # else use bnd to update normally
                        if edge_l.value != '':
                            for e in edges_added:
                                if check_consistent_edge(interpretations_edge, e, (edge_l, bnd)):
                                    u, changes = _update_edge(interpretations_edge, e, (edge_l, bnd), ipl,
                                                              rule_trace_edge, fp_cnt, t, False, convergence_mode,
                                                              atom_trace, save_graph_attributes_to_rule_trace,
                                                              rules_to_be_applied_edge_trace, idx,
                                                              facts_to_be_applied_edge_trace, rule_trace_edge_atoms,
                                                              mode='rule')
                                ...
                                # Resolve inconsistency
                                    ...
                                    resolve_inconsistency_edge(interpretations_edge, e, (edge_l, bnd), ipl, t, fp_cnt,
                                                               atom_trace, rule_trace_edge, rule_trace_edge_atoms)
                        else:
                            # Check for inconsistencies
                            if check_consistent_node(interpretations_node, comp, (l, bnd)):
                                u, changes = _update_node(interpretations_node, comp, (l, bnd), ipl, rule_trace_node,
                                                          fp_cnt, t, False, convergence_mode, atom_trace,
                                                          save_graph_attributes_to_rule_trace,
                                                          rules_to_be_applied_node_trace, idx,
                                                          facts_to_be_applied_node_trace, rule_trace_node_atoms,
                                                          mode='rule')
                            ...
                            # Resolve inconsistency
                                ...
                                resolve_inconsistency_node(interpretations_node, comp, (l, bnd), ipl, t, fp_cnt,
                                                           atom_trace, rule_trace_node, rule_trace_node_atoms)
                    ...
                    if i[0] == t:
                        ...
                        if edge_l.value != '':
                            ...
                            # Resolve inconsistency
                                ...
                                resolve_inconsistency_edge(interpretations_edge, comp, (l, bnd), ipl, t, fp_cnt,
                                                           atom_trace, rule_trace_edge, rule_trace_edge_atoms)
                ...
                if update:
                    ...
                    for rule in rules:
                        ...
                        # Only go through everything if the rule can be applied within the given timesteps. 
                        # Otherwise it's an unnecessary loop
                        if t + rule.get_delta() <= tmax:
                            for n in nodes:
                                if are_satisfied_node(interpretations_node, n,
                                                      rule.get_target_criteria()) and \
                                        is_satisfied_node(interpretations_node, n, (rule.get_target(),
                                                                                    interval.closed(0, 1))):
                                    a = neighbors[n]
                                    result, annotations, qualified_nodes, qualified_edges, edges_to_add = \
                                        _is_rule_applicable(interpretations_node, interpretations_edge, a, n,
                                                            rule.get_neigh_criteria(),
                                                            rule.get_thresholds(), reverse_graph,
                                                            rule.get_annotation_function(),
                                                            rule.get_annotation_label(), rule.get_edges(), atom_trace)
                                    if result and not interpretations_node[n].world[rule.get_target()].is_static():
                                        ...
                                        rules_to_be_applied_node.append(
                                            (numba.types.int8(t + rule.get_delta()), n, rule.get_target(), bnd))
                                        if atom_trace:
                                            rules_to_be_applied_node_trace.append(
                                                (qualified_nodes, qualified_edges, rule.get_name()))
                                        in_loop = True if rule.get_delta() == 0 else False
                            # Go through all edges and check if any rules apply to them.
                            # Comment out the following lines if there are no labels or rules that deal with edges. 
                            # It will be an unnecessary loop

                            for e in edges:
                                if are_satisfied_edge(interpretations_edge, e,
                                                      rule.get_target_criteria()) and \
                                        is_satisfied_node(interpretations_edge, e, (rule.get_target(), 
                                                                                    interval.closed(0, 1))):
                                    # Node candidates are only source and target
                                    a = numba.typed.List([e[0], e[1]])
                                    # Find out if rule is applicable. returns list of list of qualified nodes and 
                                    # qualified edges. one for each clause
                                    result, annotations, qualified_nodes, _, edges_to_add = _is_rule_applicable(
                                        interpretations_node, interpretations_edge, a, e[0], rule.get_neigh_criteria(),
                                        rule.get_thresholds(), reverse_graph, rule.get_annotation_function(),
                                        rule.get_annotation_label(), rule.get_edges(), atom_trace)
                                    if result and not interpretations_edge[e].world[rule.get_target()].is_static():
                                        bnd = influence(rule, annotations, rule.get_weights())
                                        max_rules_time = max(max_rules_time, t + rule.get_delta())
                                        edges_to_be_added_edge_rule.append(edges_to_add)
                                        rules_to_be_applied_edge.append(
                                            (numba.types.int8(t + rule.get_delta()), e, rule.get_target(), bnd))
                                        if atom_trace:
            ...
            if convergence_mode == 'delta_interpretation':
                ...
                if bound_delta <= convergence_delta:
                    if verbose:
                        print(
                            f'\nConverged at time: {t} with {float_to_str(bound_delta)} '
                            f'as the maximum bound change from the previous interpretation')
                    break
            # Perfect convergence
            # Make sure there are no rules to be applied, and no facts that will be applied in the future. 
            # We do this by checking the max time any rule/fact is applicable
...
@numba.njit(cache=True)
def _is_rule_applicable(interpretations_node,
    ...
    # If its a node clause update (x1 or x2 etc.) qualified neighbors,
    # if its an edge clause update the qualified neighbors for the source and target (x1, x2)
    ...
    for i, clause in enumerate(neigh_criteria):
        ...
        if clause[0] == 'node':
            ...
            qe = get_qualified_components_edge_clause(interpretations_edge, subset_source, subset_target, clause,
                                                      reverse_graph)
...
@numba.njit(cache=True)
def get_qualified_components_edge_clause(interpretations_edge, candidates_source, candidates_target, clause,
                                         reverse_graph):
...
@numba.njit(cache=True)
def _satisfies_threshold(num_neigh,
                         num_qualified_component,
                         threshold):
...
@numba.njit(cache=True)
def _update_node(interpretations, comp, na, ipl, rule_trace, fp_cnt, t_cnt, static, convergence_mode, atom_trace,
                 save_graph_attributes_to_rule_trace, rules_to_be_applied_trace, idx, facts_to_be_applied_trace,
                 rule_trace_atoms, mode):
    updated = False
    ...
    try:
        ...
        if updated:
            ...
            for p1, p2 in ipl:
                if p1 == l:
                    if atom_trace:
                        _update_rule_trace_node(rule_trace_atoms,
                                                numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)),
                                                numba.typed.List.empty_list(numba.typed.List.empty_list(edge_type)),
                                                world.world[p2], f'IPL: {l.get_value()}')
                    ...
                    rule_trace.append(
                        (numba.types.int8(t_cnt), numba.types.int8(fp_cnt), comp, p2, interval.closed(lower, upper)))
                if p2 == l:
                    if atom_trace:
                        _update_rule_trace_node(rule_trace_atoms,
                                                numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)),
                                                numba.typed.List.empty_list(numba.typed.List.empty_list(edge_type)),
                                                world.world[p1], f'IPL: {l.get_value()}')
                    ...
                    rule_trace.append(
                        (numba.types.int8(t_cnt), numba.types.int8(fp_cnt), comp, p1, interval.closed(lower, upper)))
...
@numba.njit(cache=True)
def _update_edge(interpretations, comp, na, ipl, rule_trace, fp_cnt, t_cnt, static, convergence_mode, atom_trace,
                 save_graph_attributes_to_rule_trace, rules_to_be_applied_trace, idx, facts_to_be_applied_trace,
                 rule_trace_atoms, mode):
    updated = False
    # This is to prevent a key error in case the label is a specific label
    try:
        ...
        if updated:
            ...
            for p1, p2 in ipl:
                if p1 == l:
                    if atom_trace:
                        _update_rule_trace_edge(rule_trace_atoms,
                                                numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)),
                                                world.world[p2], f'IPL: {l.get_value()}')
                    lower = max(world.world[p2].lower, 1 - world.world[p1].upper)
                    upper = min(world.world[p2].upper, 1 - world.world[p1].lower)
...
@numba.njit(cache=True)
def are_satisfied_node(interpretations,
                       comp,
                       nas):
    result = True
    if len(nas) > 0:
...
@numba.njit(cache=True)
def are_satisfied_edge(interpretations,
                       comp,
                       nas):
    result = True
    if len(nas) > 0:
...
@numba.njit(cache=True)
def is_satisfied_edge(interpretations,
                      comp,
                      na):
    result = False
    if not (na[0] is None or na[1] is None):
...
@numba.njit(cache=True)
def check_consistent_node(interpretations,
                          comp,
                          na):
    world = interpretations[comp]
...
@numba.njit(cache=True)
def check_consistent_edge(interpretations,
                          comp,
                          na):
    world = interpretations[comp]
    bnd = world.world[na[0]]
...
@numba.njit(cache=True)
def resolve_inconsistency_node(interpretations,
                               comp,
                               na,
                               ipl,
                               t_cnt,
    ...
    if atom_trace:
        _update_rule_trace_node(rule_trace_atoms, numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)),
                                numba.typed.List.empty_list(numba.typed.List.empty_list(edge_type)), world.world[na[0]],
                                'Inconsistency')
...
@numba.njit(cache=True)
def resolve_inconsistency_edge(interpretations,
                               comp,
                               na,
                               ipl,
                               t_cnt,
    ...
    rule_trace.append((numba.types.int8(t_cnt), numba.types.int8(fp_cnt), comp, na[0], world.world[na[0]].copy()))
    if atom_trace:
        _update_rule_trace_edge(rule_trace_atoms, numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)),
                                world.world[na[0]], 'Inconsistency')
    # Resolve inconsistency and set static
    world.world[na[0]].set_lower_upper(0, 1)
    world.world[na[0]].set_static(True)
    for p1, p2 in ipl:
...
def _add_edges(sources, 
               ...
               l, 
               interpretations_node, 
               interpretations_edge) -> (numba.typed.List, int):
    changes = 0
    edges = numba.typed.List.empty_list(edge_type)
    
    for source in sources:
        for target in targets:
            edge, new_edge = _add_edge(source, target, neighbors, nodes, edges, l, interpretations_node,
                                       interpretations_edge)
            edges.append(edge)
