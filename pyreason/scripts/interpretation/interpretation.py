...
class Interpretation:
    ...
    def __init__(self, graph, tmax, ipl, reverse_graph, atom_trace, convergence_threshold, convergence_bound_threshold):
        ...
        # One to One correspondence between rules_to_be_applied_node and rules_to_be_applied_node_trace
        # if atom_trace is true
        self.rules_to_be_applied_node_trace = numba.typed.List.empty_list(numba.types.Tuple((numba.types.ListType(
            numba.types.ListType(node_type)), numba.types.ListType(numba.types.ListType(edge_type)),
                                                                                             numba.types.string)))
        self.rules_to_be_applied_edge_trace = numba.typed.List.empty_list(
            numba.types.Tuple((numba.types.ListType(numba.types.ListType(node_type)), numba.types.string)))
        ...
        self.facts_to_be_applied_node = numba.typed.List.empty_list(numba.types.Tuple(
            (numba.types.int8, node_type, label.label_type, interval.interval_type, numba.types.boolean)))
        self.facts_to_be_applied_edge = numba.typed.List.empty_list(numba.types.Tuple(
            (numba.types.int8, edge_type, label.label_type, interval.interval_type, numba.types.boolean)))
        ...
        self.rule_trace_edge_atoms = numba.typed.List.empty_list(numba.types.Tuple(
            (numba.types.ListType(numba.types.ListType(node_type)), interval.interval_type, numba.types.string)))
        ...
        self.interpretations_node = self._init_interpretations_node(numba.typed.List(self.graph.nodes()),
                                                                    self.available_labels_node,
                                                                    self.specific_node_labels)
        self.interpretations_edge = self._init_interpretations_edge(numba.typed.List(self.graph.edges()),
                                                                    self.available_labels_edge,
    ...
    def start_fp(self, facts_node, facts_edge, rules, verbose):
        max_facts_time = self._init_facts(facts_node, facts_edge, self.facts_to_be_applied_node,
    ...
    def _init_facts(facts_node, facts_edge, facts_to_be_applied_node, facts_to_be_applied_edge,
                    facts_to_be_applied_node_trace, facts_to_be_applied_edge_trace, atom_trace):
        max_time = 0
        for fact in facts_node:
            for t in range(fact.get_time_lower(), fact.get_time_upper() + 1):
                max_time = max(max_time, t)
                facts_to_be_applied_node.append(
                    (numba.types.int8(t), fact.get_component(), fact.get_label(), fact.get_bound(), fact.static))
                if atom_trace:
    ...
    def _start_fp(self, rules, max_facts_time, verbose):
        fp_cnt, t = self.reason(self.interpretations_node, self.interpretations_edge, self.tmax, rules,
                                numba.typed.List(self.graph.nodes()), numba.typed.List(self.graph.edges()),
                                self.neighbors, self.rules_to_be_applied_node, self.rules_to_be_applied_edge,
                                self.edges_to_be_added_node_rule, self.edges_to_be_added_edge_rule,
                                self.rules_to_be_applied_node_trace, self.rules_to_be_applied_edge_trace,
                                self.facts_to_be_applied_node, self.facts_to_be_applied_edge,
                                self.facts_to_be_applied_node_trace, self.facts_to_be_applied_edge_trace,
                                self.available_labels_node, self.available_labels_edge, self.specific_node_labels,
                                self.specific_edge_labels, self.ipl, self.rule_trace_node, self.rule_trace_edge,
                                self.rule_trace_node_atoms, self.rule_trace_edge_atoms, self.reverse_graph,
                                self.atom_trace, max_facts_time, self._convergence_mode, self._convergence_delta,
                                verbose)
        self.time = t
        if verbose:
    ...
    @staticmethod
    @numba.njit(cache=True)
    def reason(interpretations_node, interpretations_edge, tmax, rules, nodes, edges, neighbors,
               ...
               ipl, rule_trace_node, rule_trace_edge, rule_trace_node_atoms, rule_trace_edge_atoms, reverse_graph,
               atom_trace, max_facts_time, convergence_mode, convergence_delta, verbose):
        fp_cnt = 0
        for t in range(tmax + 1):
            ...
            for i in range(len(facts_to_be_applied_node)):
                if facts_to_be_applied_node[i][0] == t:
                    comp, l, bnd, static = facts_to_be_applied_node[i][1], facts_to_be_applied_node[i][2], \
                        facts_to_be_applied_node[i][3], facts_to_be_applied_node[i][4]
                    # Check if bnd is static. Then no need to update, just add to rule trace,
                    # and add ipl complement to rule trace as well
                    if interpretations_node[comp].world[l].is_static():
                        ...
                        if atom_trace:
                            _update_rule_trace_node(rule_trace_node_atoms,
                                                    ...
                                                    bnd, facts_to_be_applied_node_trace[i])
                        for p1, p2 in ipl:
                            if p1 == l:
                                ...
                                if atom_trace:
                                    _update_rule_trace_node(rule_trace_node_atoms, numba.typed.List.empty_list(
                                        ...
                                        numba.typed.List.empty_list(edge_type)), interpretations_node[comp].world[p2],
                                ...
                                if atom_trace:
                                    _update_rule_trace_node(rule_trace_node_atoms, numba.typed.List.empty_list(
                                        numba.typed.List.empty_list(node_type)), numba.typed.List.empty_list(
                                        numba.typed.List.empty_list(edge_type)), interpretations_node[comp].world[p1],
                                                            facts_to_be_applied_node_trace[i])
                        ...
                        if check_consistent_node(interpretations_node, comp, (l, bnd)):
                            u, changes = _update_node(interpretations_node, comp, (l, bnd), ipl, rule_trace_node,
                                                      ...
                                                      rule_trace_node_atoms, mode='fact')
                    ...
                    if static:
                        facts_to_be_applied_node[i] = (
                            numba.types.int8(facts_to_be_applied_node[i][0] + 1), comp, l, bnd, static)
            ...
            for i in range(len(facts_to_be_applied_edge)):
                if facts_to_be_applied_edge[i][0] == t:
                    comp, l, bnd, static = facts_to_be_applied_edge[i][1], facts_to_be_applied_edge[i][2], \
                        facts_to_be_applied_edge[i][3], facts_to_be_applied_edge[i][4]
                    # Check if bnd is static. Then no need to update, just add to rule trace,
                    # and add ipl complement to rule trace as well
                    if interpretations_edge[comp].world[l].is_static():
                        ...
                        if atom_trace:
                            _update_rule_trace_edge(rule_trace_edge_atoms,
                                                    numba.typed.List.empty_list(numba.typed.List.empty_list(node_type)),
                                                    bnd, facts_to_be_applied_edge_trace[i])
                        for p1, p2 in ipl:
                            if p1 == l:
                                ...
                                if atom_trace:
                                    _update_rule_trace_edge(rule_trace_edge_atoms, numba.typed.List.empty_list(
                                        numba.typed.List.empty_list(node_type)), interpretations_edge[comp].world[p2],
                                ...
                                if atom_trace:
                                    _update_rule_trace_edge(rule_trace_edge_atoms, numba.typed.List.empty_list(
                                        numba.typed.List.empty_list(node_type)), interpretations_edge[comp].world[p1],
                                                            facts_to_be_applied_edge_trace[i])
                    else:
                        ...
                        if check_consistent_edge(interpretations_edge, comp, (l, bnd)):
                            u, changes = _update_edge(interpretations_edge, comp, (l, bnd), ipl, rule_trace_edge,
                                                      ...
                                                      rule_trace_edge_atoms, mode='fact')
                    ...
                    if static:
                        facts_to_be_applied_edge[i] = (
                            numba.types.int8(facts_to_be_applied_edge[i][0] + 1), comp, l, bnd, static)
                            ...
                            for e in edges_added:
                                if check_consistent_edge(interpretations_edge, e, (edge_l, bnd)):
                                    u, changes = _update_edge(interpretations_edge, e, (edge_l, bnd), ipl,
                                                              ...
                                                              atom_trace, rules_to_be_applied_edge_trace, idx,
                        ...
                        if t + rule.get_delta() <= tmax:
                            for n in nodes:
                                if are_satisfied_node(interpretations_node, n,
                                        ...
                                        is_satisfied_node(interpretations_node, n,
                                                          (rule.get_target(), interval.closed(0, 1))):
                                    a = neighbors[n]
                                    result, annotations, qualified_nodes, qualified_edges, edges_to_add = _is_rule_applicable(
                                        interpretations_node, interpretations_edge, a, n, rule.get_neigh_criteria(),
                            ...
                            for e in edges:
                                if are_satisfied_edge(interpretations_edge, e,
                                                      rule.get_target_criteria()) and is_satisfied_node(
                                    interpretations_edge, e, (rule.get_target(), interval.closed(0, 1))):
...
@numba.njit(cache=True)
def _is_rule_applicable(interpretations_node, interpretations_edge, candidates, target_node, neigh_criteria, thresholds,
                        reverse_graph, ann_fn, ann_fn_label, edges, atom_trace):
...
def _update_node(interpretations, comp, na, ipl, rule_trace, fp_cnt, t_cnt, static, convergence_mode, atom_trace,
                 rules_to_be_applied_trace, idx, facts_to_be_applied_trace, rule_trace_atoms, mode):
    updated = False
