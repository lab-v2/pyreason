"""
Example: Cybersecurity Inconsistency Tutorial
==============================================
...
Three types of inconsistency are demonstrated:
  ...
  2. Inconsistent Predicate List (IPL) conflict via facts:
     "vulnerable" and "patched" are declared as mutually exclusive.
     Asserting both for the same asset via facts creates a contradiction
     which PyReason resolves to [0.0, 1.0] on both predicates.

  3. Rule-triggered inconsistency via same-predicate non-overlapping bounds:
     A four-hop rule chain infers patch_confidence(dev_server):[0.0,0.2].
     This conflicts with the dev_patch_db_fact asserting
     patch_confidence(dev_server):[0.9,1.0] -- non-overlapping bounds,
     rule-triggered inconsistency.
     ...
     Rule chain:
       ...
       unpatched_rule     -> patch_confidence(dev_server):[0.0,0.2]
                                   conflicts with dev_patch_db_fact:[0.9,1.0]

Real CVEs used:
  - cve_2021_3156   sudo 1.9.5p1      CVSS 7.8  CWE-121 (heap buffer overflow)
  - cve_2022_0185   Linux Kernel 5.1  CVSS 8.4  CWE-121 (stack overflow)
  - cve_2022_26923  OpenSSL 3.0.1     CVSS 7.5  CWE-415 (double free)
"""
...
# Variables must be uppercase per PyReason convention
pr.add_rule(pr.Rule(
...
# Rule 3: A vulnerable asset is also compromised with high confidence
pr.add_rule(pr.Rule(
...
# Asset management DB says dev_server has high patch confidence.
# The rule chain (exposure -> vulnerability -> compromise -> unpatched_rule)
# will infer patch_confidence(dev_server):[0.0,0.2].
# [0.0, 0.2] and [0.9, 1.0] do not overlap -- rule-triggered inconsistency.
pr.add_fact(pr.Fact('patch_confidence(dev_server):[0.9,1.0]', 'dev_patch_db_fact', 0, 3))

# ================================ REASON ======================================
print('=' * 60)
print('Running PyReason -- Cybersecurity Inconsistency Tutorial')
print('=' * 60)
...
print('\n' + '=' * 60)
print('Vulnerable / Patched (Demo 2: IPL fact conflict on web_server)')
print('=' * 60)
...
print('\n' + '=' * 60)
print('Patch Confidence (Demo 3: rule-triggered conflict on dev_server)')
print('=' * 60)
dataframes = pr.filter_and_sort_nodes(interpretation, ['patch_confidence', 'compromised'])
for t, df in enumerate(dataframes):
    print(f'\nTIMESTEP {t}:')
    print(df)
...
# Save the rule trace to csv_outputs/ for further inspection
